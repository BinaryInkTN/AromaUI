...
tm_repl = """    case EVENT_TYPE_MOUSE_MOVE:
    case EVENT_TYPE_TOUCH_MOVE:
    {
        ...
        if (event->event_type == EVENT_TYPE_MOUSE_MOVE) {
            tx = event->data.mouse.x; ty = event->data.mouse.y; id = 0;
            // Also simulate scroll wheel support using delta_y/dx?
            // Actually, we check if button 0 is down or if it's dragging pointer ID 0
            // The original code uses EVENT_TYPE_MOUSE_MOVE for scroll wheel (delta_x, delta_y).
            // Let's preserve scroll wheel if dragging is NOT active.
        } else {
            tx = event->data.touch.x; ty = event->data.touch.y; id = event->data.touch.id;
        }
...
# we need to remove the tx = and ty =
text = text.replace("        int tx = event->data.touch.x;\n        int ty = event->data.touch.y;", "")
...
text = text.replace("event->data.touch.x", "tx")
text = text.replace("event->data.touch.y", "ty")

# We should also remove the old EVENT_TYPE_MOUSE_MOVE: block entirely down below.
import re
