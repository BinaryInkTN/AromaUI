                             ...
                             uint32_t color, float thickness, bool round_cap)
{
    ...
    if (list->count >= list->capacity) return;
    AromaDrawCmd* cmd = &list->commands[list->count++];
    cmd->type = AROMA_DRAW_CMD_LINE;
    cmd->data.line.x0 = x0;
    cmd->data.line.y0 = y0;
    cmd->data.line.x1 = x1;
    cmd->data.line.y1 = y1;
    cmd->data.line.color = color;
    cmd->data.line.thickness = thickness;
    cmd->data.line.round_cap = round_cap;
    cmd->is_drawn = false;
}
    ...
    for (size_t i = 0; i < list->count; i++) {
        ...
        switch (cmd->type) {
            ...
            case AROMA_DRAW_CMD_ARC:
                ...
                break;
            case AROMA_DRAW_CMD_LINE:
                if (gfx->draw_line)
                    gfx->draw_line(window_id,
                                   cmd->data.line.x0,
                                   cmd->data.line.y0,
                                   cmd->data.line.x1,
                                   cmd->data.line.y1,
                                ...
                                int x, int y, int width, int height)
{
    ...
    for (size_t i = 0; i < list->count; i++) {
        ...
        switch (cmd->type) {
            ...
            case AROMA_DRAW_CMD_LINE:
                if (gfx->draw_line) {
                    int min_x = cmd->data.line.x0 < cmd->data.line.x1 ? cmd->data.line.x0 : cmd->data.line.x1;
                    int min_y = cmd->data.line.y0 < cmd->data.line.y1 ? cmd->data.line.y0 : cmd->data.line.y1;
                    int max_x = cmd->data.line.x0 > cmd->data.line.x1 ? cmd->data.line.x0 : cmd->data.line.x1;
                    int max_y = cmd->data.line.y0 > cmd->data.line.y1 ? cmd->data.line.y0 : cmd->data.line.y1;
                    if (rect_intersects(min_x, min_y, max_x - min_x + 1, max_y - min_y + 1, x, y, width, height)) {
                        gfx->draw_line(window_id,
                                       cmd->data.line.x0,
                                       cmd->data.line.y0,
                                       cmd->data.line.x1,
