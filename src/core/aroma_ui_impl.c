...
static int __draw_task_compare(const void* a, const void* b)
{
    const AromaDrawTask* ta = (const AromaDrawTask*)a;
    const AromaDrawTask* tb = (const AromaDrawTask*)b;
    if (ta->z_index < tb->z_index) return -1;
    if (ta->z_index > tb->z_index) return 1;
    return 0;
}

static int __find_window_index_by_id(size_t window_id)
{
...
static bool __node_matches_window_id(AromaNode* node, size_t window_id)
{
...
bool aroma_ui_init_impl(void) {
    ...
    if (immediate_env && immediate_env[0] == '1') {
...
AromaDrawList* aroma_ui_begin_frame(size_t window_id)
{
    ...
    if (!list) {
        list = aroma_drawlist_create();
        g_window_drawlists[idx] = list;
    }
...
void aroma_ui_render_dirty_window(size_t window_id, uint32_t clear_color)
{
    ...
    if (aroma_backend_abi.get_graphics_backend_type &&
        aroma_backend_abi.get_graphics_backend_type() == GRAPHICS_BACKEND_GLES3) {
        for (int i = 0; i < g_window_count; i++) {
            if (g_windows[i].window_id != window_id) continue;
            AromaNode* root = g_windows[i].root_node;
            if (!root) continue;
