    ...
    if (event->event_type == EVENT_TYPE_MOUSE_CLICK || event->event_type == EVENT_TYPE_TOUCH_DOWN) {
        ...
        if (my > t->header_height) {
            ...
            if (row >= 0 && row < t->num_rows) {
                ...
                if (t->callback) t->callback(row, t->user_data);
                return true;
            }
        }
    }
...
int aroma_table_add_row(AromaNode* table_node) {
    AromaTableInternal* t = get_table(table_node);
    if (!t || t->num_rows >= MAX_ROWS) return -1;
    int row = t->num_rows++;
...
void aroma_table_draw(AromaNode* table_node, size_t window_id) {
    AromaTableInternal* t = get_table(table_node);
    if (!t || aroma_node_is_hidden(table_node)) return;
    
    AromaGraphicsInterface* gfx = aroma_backend_abi.get_graphics_interface();
    if (!gfx || !gfx->fill_rectangle || !gfx->render_text || !t->font) return;
    
    AromaTheme theme = aroma_theme_get_global();
    int cur_y = t->rect.y;
    int total_w = 0;
    for (int i=0; i<t->num_cols; i++) total_w += t->col_widths[i];
    ...
    for (int c = 0; c < t->num_cols; c++) {
        // line separator
        if (c > 0) gfx->fill_rectangle(window_id, cur_x, cur_y, 1, t->rect.height, theme.colors.border, false, 0);
