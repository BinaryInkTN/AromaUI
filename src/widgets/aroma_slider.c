...
height = aroma_android_dp_to_px(height);
    ...
    data->thumb_hover_color = data->thumb_color;
...
void aroma_slider_set_value(AromaNode* node, int value)
{
    ...
    if (data->current_value != value) {
        ...
        data->thumb_x = data->rect.x + (data->current_value - data->min_value) * data->rect.width / range;
        if (data->on_change) {
...
void aroma_slider_on_click(AromaNode* node, int mouse_x, int mouse_y)
{
    ...
    if (mouse_x >= data->rect.x && mouse_x <= data->rect.x + data->rect.width &&
        ...
        int range = data->max_value - data->min_value;
        aroma_slider_set_value(node, data->min_value + (click_offset * range) / data->rect.width);
    }
}
...
void aroma_slider_draw(AromaNode* node, size_t window_id)
{
    ...
    gfx->fill_rectangle(window_id, data->rect.x, track_y, data->rect.width, data->track_height,
                       data->track_color, true, data->track_corner_radius);
