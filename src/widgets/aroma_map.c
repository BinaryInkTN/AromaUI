    ...
    *count = 0;
    ...
    while (*count < MAX_GEOCODE_RESULTS)
    {
        ...
        while (*name_end && *name_end != '"')
        {
            ...
            else if ((*name_end & 0xE0) == 0xC0)
            {
                name_end += 2;
            }
            else if ((*name_end & 0xF0) == 0xE0)
            {
                name_end += 3;
            }
            else if ((*name_end & 0xF8) == 0xF0)
            {
                name_end += 4;
            }
            else
        ...
        if (city)
        {
            ...
            if (city)
            {
                ...
                while (*city_end && *city_end != '"')
                {
                    if ((*city_end & 0x80) == 0)
                        city_end++;
                    else if ((*city_end & 0xE0) == 0xC0)
                        city_end += 2;
                    else if ((*city_end & 0xF0) == 0xE0)
                        city_end += 3;
                    else
                        city_end++;
                }
        ...
        if (results[*count].category[0] == '\0')
        {
            ...
            if (country)
            {
                ...
                if (country)
                {
                    ...
                    {
                        if ((*country_end & 0x80) == 0)
                            country_end++;
                        else if ((*country_end & 0xE0) == 0xC0)
                            country_end += 2;
                        else if ((*country_end & 0xF0) == 0xE0)
                            country_end += 3;
                        else
                            country_end++;
                    }
...
static void decode_polyline(const char *encoded, double **lats, double **lons, int *count)
{
    ...
    *count = 0;
    ...
    while (index < len)
    {
        ...
        {
            cap *= 2;
            double *new_lats = realloc(*lats, cap * sizeof(double));
            double *new_lons = realloc(*lons, cap * sizeof(double));
...
static void __map_route_fetch_success(emscripten_fetch_t *fetch)
{
    ...
    if (req && req->extra && fetch->data && fetch->numBytes > 0)
    {
...
static void _map_draw_line(AromaGraphicsInterface *gfx, size_t window_id, int x0, int y0, int x1, int y1, uint32_t color, int thickness, int cx, int cy, int cw, int ch)
{
    ...
    if ((y1 - y0) == 0 && (x1 - x0) == 0)
    {
        ...
        {
            gfx->fill_rectangle(window_id, x0 - thickness / 2, y0 - thickness / 2, thickness, thickness, color, true, thickness / 2.0f);
        }
        return;
    }

    int outcode0 = (((x0) < cx) ? 1 : ((x0) > cx + cw) ? 2 : 0) |
                   (((y0) < cy) ? 4 : ((y0) > cy + ch) ? 8 : 0);
    int outcode1 = (((x1) < cx) ? 1 : ((x1) > cx + cw) ? 2 : 0) |
                   (((y1) < cy) ? 4 : ((y1) > cy + ch) ? 8 : 0);
    while (1)
    {
    ...
    for (int i = 0; i < extra->marker_count; i++)
    {
            ...
            draw_y >= map->rect.y && draw_y <= map->rect.y + map->rect.height)
        {
            ...
            if (extra->active_popup_idx == i && extra->markers[i].popup_text && extra->font && gfx && gfx->render_text)
            {
                ...
                if (bg_x < map->rect.x)
                    bg_x = map->rect.x;
                if (bg_x + bg_w > map->rect.x + map->rect.width)
                    bg_x = map->rect.x + map->rect.width - bg_w;
...
height = aroma_android_dp_to_px(height);
    ...
    if (!curl_initialized)
    {
        ...
        for (int i = 0; i < num_active_workers; i++)
        {
            ...
            {
                /* Thread creation failed, continue with fewer workers */
            }
            else
            {
