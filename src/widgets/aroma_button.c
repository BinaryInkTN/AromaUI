...
{
    if (!button || button->label[0] == '\0')
        return;

    AromaGraphicsInterface *gfx = aroma_backend_abi.get_graphics_interface();
    if (!gfx || !gfx->measure_text)
        return;
...
bool aroma_button_handle_mouse_event(AromaNode *button_node, int mouse_x, int mouse_y, bool is_clicked)
{
    ...
    int adjusted_x = mouse_x;
    int adjusted_y = mouse_y;

    AromaNode *cur = button_node->parent_node;
    while (cur)
    {
    ...
    bool is_in_bounds = aroma_button_point_in_bounds(button, adjusted_x, adjusted_y);
