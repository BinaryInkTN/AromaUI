...
void aroma_radio_button_draw(AromaNode* node, size_t window_id)
{
    ...
    int circle_x = data->rect.x;
    int circle_y = data->rect.y + (data->rect.height - diameter) / 2;
    ...
    gfx->fill_rectangle(window_id, circle_x, circle_y, diameter, diameter,
                        data->background_color, true, diameter / 2.0f);
    gfx->draw_hollow_rectangle(window_id, circle_x, circle_y, diameter, diameter,
                               ring_color, 1.0f, true, diameter / 2.0f);

    if (data->is_selected) {
        int inner = diameter - 8;
        int inner_x = circle_x + (diameter - inner) / 2;
        int inner_y = circle_y + (diameter - inner) / 2;
        gfx->fill_rectangle(window_id, inner_x, inner_y, inner, inner,
                            data->dot_color, true, inner / 2.0f);
    }

    if (data->font && data->label[0] != '\0' && gfx->render_text) {
        int text_x = circle_x + diameter + 10;
