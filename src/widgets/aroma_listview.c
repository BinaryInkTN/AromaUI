...
{
    /* Safe fallback — never dereferences list when it's NULL. */
    if (!list) return AROMA_LIST_MIN_ITEM_HEIGHT;
    if (!item_in_range(list, i)) return list->item_height;
    if (is_header(list, i))    return list->item_height / 2;
    if (is_separator(list, i)) return 1;
...
static int hit_test(const AromaListViewInternal *list, int screen_y)
{
    ...
    for (size_t i = 0; i < list->item_count; i++) {
        ...
        if (rel_y >= cy && rel_y < cy + ih) return (int)i;
        cy += ih;
    }
    return -1;
}

/* Shared selection commit — avoids duplicating callback + vibrate logic. */
static void commit_selection(AromaListViewInternal *list, AromaNode *node, int hit)
{
    ...
    if (plat && plat->android_vibrate) plat->android_vibrate(60);
...
{
    (void)user_data;
    if (!ev || !ev->target_node) return false;
    ...
    case EVENT_TYPE_TOUCH_UP: {
        if (ev->data.touch.id != list->active_pointer_id) return false;
        list->active_pointer_id = -1;
                                  ...
                                  int width, int height)
{
    ...
    if (!node) { aroma_widget_free(list); return NULL; }
...
/* Shared string copy with guaranteed NUL-termination. */
...
{
    if (list->item_count >= AROMA_LIST_MAX_ITEMS) return NULL;
...
void aroma_listview_add_separator(AromaNode *node)
{
    ...
    AromaListViewInternal *list = get_internal(node);
    if (!list) return;
    if (!reserve_item(list, AROMA_LIST_ITEM_SEPARATOR)) return;
    update_content_height(list);
    aroma_node_invalidate(node);
}
...
int    aroma_listview_get_selected(AromaNode *n)
       { AromaListViewInternal *l = get_internal(n); return l ? l->selected_index : -1; }
size_t aroma_listview_get_count(AromaNode *n)
       { AromaListViewInternal *l = get_internal(n); return l ? l->item_count : 0; }
...
{
    AromaListViewInternal *l = get_internal(n);
    if (l) { l->callback = cb; l->user_data = ud; }
}
...
{ AromaListViewInternal *l = get_internal(n); if (l) { l->secondary_font = f; aroma_node_invalidate(n); } }

void aroma_listview_set_icon_font(AromaNode *n, AromaFont *f)
{ AromaListViewInternal *l = get_internal(n); if (l) { l->icon_font = f; aroma_node_invalidate(n); } }
...
{ AromaListViewInternal *l = get_internal(n); if (l) { l->text_scale = s; aroma_node_invalidate(n); } }

void aroma_listview_set_secondary_text_scale(AromaNode *n, float s)
{ AromaListViewInternal *l = get_internal(n); if (l) { l->secondary_text_scale = s; aroma_node_invalidate(n); } }

void aroma_listview_set_corner_radius(AromaNode *n, float r)
{ AromaListViewInternal *l = get_internal(n); if (l) { l->corner_radius = r; aroma_node_invalidate(n); } }

void aroma_listview_set_selected_corner_radius(AromaNode *n, float r)
{ AromaListViewInternal *l = get_internal(n); if (l) { l->selected_corner_radius = r; aroma_node_invalidate(n); } }

void aroma_listview_show_headers(AromaNode *n, bool show)
{ AromaListViewInternal *l = get_internal(n); if (l) { l->show_headers = show; aroma_node_invalidate(n); } }

void aroma_listview_set_header_colors(AromaNode *n, uint32_t bg, uint32_t text)
{
    AromaListViewInternal *l = get_internal(n);
    if (l) { l->header_bg_color = bg; l->header_text_color = text; aroma_node_invalidate(n); }
}

void aroma_listview_set_scroll_container(AromaNode *n, AromaNode *c)
{ AromaListViewInternal *l = get_internal(n); if (l) l->scroll_container = c; }

AromaNode *aroma_listview_get_scroll_container(AromaNode *n)
{ AromaListViewInternal *l = get_internal(n); return l ? l->scroll_container : NULL; }

/* -------------------------------------------------------------------------
 * Draw  (pure read — must never call invalidate)
 * ---------------------------------------------------------------------- */

void aroma_listview_draw(AromaNode *node, size_t window_id)
{
    if (!node || aroma_node_is_hidden(node)) return;

    AromaListViewInternal *list = get_internal(node);
    if (!list || !list->font) {
        if (list) LOG_ERROR("listview draw: font is NULL");
        return;
    }
