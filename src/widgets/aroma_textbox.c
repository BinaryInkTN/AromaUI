...
{
    if (!textbox || length == 0) return 0.0f;
    if (!gfx || !gfx->measure_text || !textbox->font || window_id == SIZE_MAX) {
        return (float)(length * 8);
    }
    if (length > textbox->text_length) length = textbox->text_length;
    char buffer[AROMA_TEXTBOX_MAX_LENGTH];
    memcpy(buffer, textbox->text, length);
    buffer[length] = '\0';
    return gfx->measure_text(window_id, textbox->font, buffer, textbox->text_scale);
}
                                          ...
                                          size_t window_id, int mouse_x)
{
    ...
    if (!gfx || !gfx->measure_text || !textbox->font || window_id == SIZE_MAX || textbox->text_length == 0) {
        ...
        if (approx > textbox->text_length) approx = textbox->text_length;
        return approx;
    }
    float accumulated = 0.0f;
    for (size_t i = 0; i < textbox->text_length; ++i) {
        char glyph[2] = { textbox->text[i], '\0' };
        float advance = gfx->measure_text(window_id, textbox->font, glyph, textbox->text_scale);
        float midpoint = accumulated + (advance * 0.5f);
        if (relative_x < midpoint) return i;
        accumulated += advance;
        if (relative_x < accumulated) return i + 1;
    }
...
{
    if (!node || !node->node_widget_ptr || !placeholder) return;
...
{
    if (!node || !node->node_widget_ptr || !text) return;
    AromaTextbox* data = (AromaTextbox*)node->node_widget_ptr;
...
void aroma_textbox_on_click(AromaNode* node, int mouse_x, int mouse_y)
{
    ...
    if (inside) {
        ...
        if (desired_cursor > data->text_length) desired_cursor = data->text_length;
...
static void __textbox_update_cursor_position(AromaTextbox* data, AromaGraphicsInterface* gfx, size_t window_id)
{
    if (!data->font) return;
    int line_height = aroma_font_get_line_height(data->font);
    if (line_height <= 0) line_height = data->rect.height - 4;
    if (line_height < 2) line_height = 2;
    data->cursor_height = line_height;
    data->cursor_y = data->rect.y + (data->rect.height - data->cursor_height) / 2;

    float prefix_width = __textbox_measure_prefix(data, gfx, window_id, data->cursor_pos);
    data->cursor_x = data->text_x + (int)(prefix_width + 0.5f);
    if (data->cursor_x > data->rect.x + data->rect.width - 2) {
        data->cursor_x = data->rect.x + data->rect.width - 2;
    }
}
...
void aroma_textbox_draw(AromaNode* node, size_t window_id)
{
    ...
    else if (data->is_hovered) fill_color = data->hover_bg_color;

    uint32_t border_color = data->border_color;
    if (data->is_focused) border_color = data->focused_border_color;
    else if (data->is_hovered) border_color = data->hover_border_color;
    ...
    if (data->font && gfx->render_text) {
        ...
        if (gfx->measure_text && available_width > 0) {
            ...
            if (full_width > (float)available_width) {
                const char* ellipsis = "…";
                float ellipsis_width = gfx->measure_text(window_id, data->font, ellipsis, data->text_scale);
    ...
    if (data->is_focused) {
        ...
        if (data->cursor_blink_time == 0) data->cursor_blink_time = now;
        ...
        if (data->show_cursor) {
            __textbox_update_cursor_position(data, gfx, window_id);
            gfx->fill_rectangle(window_id, data->cursor_x, data->cursor_y, 2, data->cursor_height,
        ...
        case EVENT_TYPE_KEY_PRESS:
            {
                ...
                if (key_code < 32 || key_code > 255) return true;
