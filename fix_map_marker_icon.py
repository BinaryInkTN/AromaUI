with open("include/widgets/aroma_map.h", "r") as f:
    text = f.read()

text = text.replace("void aroma_map_add_marker(AromaNode* node, double lat, double lon, uint32_t color);",
...
with open("include/widgets/aroma_map.h", "w") as f:
    f.write(text)
...
# Find __map_draw function to render icon
# Let's use regex to find where markers are drawn

import re
# The markers drawing loop
# extra->markers[i]
...
add_func = """void aroma_map_add_marker(AromaNode* node, double lat, double lon, uint32_t color) {
    if (!node || node->node_type != AROMA_NODE_TYPE_CUSTOM) return;
    AromaMap* map = (AromaMap*)node->data;
