...
static void window_update_callback(size_t window_id, void* data) {
    ...
    if (dirty_count > 0) {
        if (fab) aroma_fab_draw(fab, window_id);
...
int main(void) {
    ...
    AromaWindow* window = aroma_ui_create_window("Widget Example - FAB", 300, 240);
    if (!window) return 1;

    aroma_event_set_root((AromaNode*)window);
    if (font) aroma_ui_prepare_font_for_window(0, font);

    fab = aroma_fab_create((AromaNode*)window, 120, 80, FAB_SIZE_NORMAL, "+");
    if (fab) {
        aroma_fab_set_font(fab, font);
        aroma_node_invalidate(fab);
    }
