...
static const char *s_subtitles[SETUP_PAGE_COUNT] = {
    ...
    "Join a network for maps, store downloads and updates.",
...
static bool on_setup_next(AromaNode *node, void *user_data)
{
    ...
    if (!s_active)
        return true;
    /* Persist the page being left. */
    if (s_page == 1)
    {
        setup_store_set("device_name", s_device_name[0] ? s_device_name : "Aroma Infotainment");
        bt_speaker_set_device_name(setup_store_get("device_name", "Aroma Infotainment"));
    }
    else if (s_page == 2)
    {
...
/* Quote one shell word safely for popen(). */
...
static void *wifi_connect_thread(void *arg)
{
    ...
    /* Credentials persist no matter what - the car remembers the network. */
    setup_store_set("wifi_ssid", ssid);
    setup_store_set("wifi_pass", pass);
    setup_store_save();
#ifdef __EMSCRIPTEN__
    /* No subprocesses on web: persisting is all we can do. */
    wifi_set_status("Saved.");
    return NULL;
#else
    /* Is there a network manager to talk to? */
...
static void setup_build(void)
{
    ...
    /* Page 1: device name. */
    {
        ...
        /* Pre-fill is handled by the stored default above on Next; show it. */
    ...
    /* Apply persisted/derived state before first paint. */
