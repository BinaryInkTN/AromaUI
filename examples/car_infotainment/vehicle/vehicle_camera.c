...
void apply_vehicle_model_to_viewer(Aroma3DModel *model, float metallic, float clearcoat)
{
    ...
    Aroma3DCamera current;
    bool have_current = has_locked_vehicle_camera && aroma_3d_viewer_get_camera(state.viewer_3d, &current);
    bool mid_startup = state.startup_animating && state.camera_animating;

    apply_vehicle_paint_finish(model, metallic, clearcoat);
    aroma_3d_viewer_set_model(state.viewer_3d, model);

    Aroma3DCamera resting;
    resting.theta = VEHICLE_CAM_EXTERIOR_THETA;
    resting.phi = VEHICLE_CAM_EXTERIOR_PHI;
    resting.radius = VEHICLE_CAM_EXTERIOR_RADIUS;
    resting.target[0] = VEHICLE_CAM_EXTERIOR_TARGET_X;
    resting.target[1] = VEHICLE_CAM_EXTERIOR_TARGET_Y;
    resting.target[2] = VEHICLE_CAM_EXTERIOR_TARGET_Z;
    resting.fov = 45.0f;
    resting.near_plane = 0.1f;
    resting.far_plane = 1000.0f;

    if (!have_current)
    {
        Aroma3DCamera start;
        start.theta = VEHICLE_CAM_FRONT_THETA;
        start.phi = VEHICLE_CAM_FRONT_PHI;
        start.radius = VEHICLE_CAM_FRONT_RADIUS;
        start.target[0] = VEHICLE_CAM_FRONT_TARGET_X;
        start.target[1] = VEHICLE_CAM_FRONT_TARGET_Y;
        start.target[2] = VEHICLE_CAM_FRONT_TARGET_Z;
        start.fov = 45.0f;
        start.near_plane = 0.1f;
        start.far_plane = 1000.0f;
        memcpy(&locked_vehicle_camera, &resting, sizeof(Aroma3DCamera));
        has_locked_vehicle_camera = true;
        aroma_3d_viewer_set_camera(state.viewer_3d, &start);
        state.startup_animating = true;
        state.camera_animating = true;
        state.anim_target_theta = resting.theta;
        state.anim_target_phi = resting.phi;
        state.anim_target_radius = resting.radius;
        state.anim_target_x = resting.target[0];
        state.anim_target_y = resting.target[1];
        state.anim_target_z = resting.target[2];
    ...
    {
        memcpy(&locked_vehicle_camera, &current, sizeof(Aroma3DCamera));
        has_locked_vehicle_camera = true;
        aroma_3d_viewer_set_camera(state.viewer_3d, &current);
...
{
    aroma_node_set_hidden(state.vehicle_view_clock_label, hidden);
    aroma_node_set_hidden(state.vehicle_view_clock_date_label, hidden);
