...
static void read_os_release_field(const char *field, char *value, size_t size)
{
    FILE *f = fopen("/etc/os-release", "r");
    if (!f) { snprintf(value, size, "Unknown"); return; }
    ...
    while (fgets(line, sizeof(line), f))
    {
        if (strncmp(line, field, field_len) == 0 && line[field_len] == '=')
        {
            ...
            if (len > 0 && val[len-1] == '\n') { val[len-1] = '\0'; len--; }
            if (len >= 2 && val[0] == '"' && val[len-1] == '"') { val++; len -= 2; val[len] = '\0'; }
            truncate_str(val, value, size);
            fclose(f);
            return;
        }
    }
...
static void read_kernel_version(char *buf, size_t size)
{
    FILE *f = fopen("/proc/version", "r");
    if (!f) { snprintf(buf, size, "Unknown"); return; }
    
    char raw[512];
    if (!fgets(raw, sizeof(raw), f)) { snprintf(buf, size, "Unknown"); }
    else
    {
        char *nl = strchr(raw, '\n'); if (nl) *nl = '\0';
        char *space = strchr(raw, ' ');
        if (space) *space = '\0';
        truncate_str(raw, buf, size);
    }
...
static void read_cpu_model(char *buf, size_t size)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        if (strncmp(line, "model name", 10) == 0)
        {
            ...
            {
                char raw[512];
                snprintf(raw, sizeof(raw), "%s", colon + 2);
                char *nl = strchr(raw, '\n'); if (nl) *nl = '\0';
                truncate_str(raw, buf, size);
                fclose(f);
                return;
...
static void read_meminfo(const char *field, char *buf, size_t size)
{
    FILE *f = fopen("/proc/meminfo", "r");
    if (!f) { snprintf(buf, size, "Unknown"); return; }
    ...
    while (fgets(line, sizeof(line), f))
    {
        if (strncmp(line, field, field_len) == 0)
        {
            ...
            else { snprintf(buf, size, "Unknown"); }
            fclose(f);
            return;
        }
    }
...
static AromaNode *build_about_page_content(AromaNode *parent, int panel_w)
{
    int card_w = panel_w - 20;
    int y = 10;

    AromaNode *banner = aroma_ui_card(parent, 10, y, card_w, 200, CARD_TYPE_OUTLINED);
    aroma_ui_image(banner,
#ifdef __EMSCRIPTEN__
        "/assets/aromaos_logo.png"
#elif defined(__arm__) || defined(__aarch64__)
        "/usr/share/infotainment/assets/aromaos_logo.png"
#else
        "../assets/aromaos_logo.png"
#endif
        , card_w / 2 - 48, 40, 96, 96);
    aroma_ui_label(banner, "Based on Aroma Framework: v0.0.1", card_w / 2 - 140, 160, LABEL_STYLE_LABEL_LARGE, state.settings_font);
    y += 210;
char buf[128];
    read_kernel_version(buf, sizeof(buf));
    AromaNode *c = aroma_ui_card(parent, 10, y, card_w, 55, CARD_TYPE_GLASS);
    aroma_ui_label(c, "Kernel", 16, 8, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    aroma_ui_label(c, buf, 16, 28, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    y += 65;

    read_cpu_model(buf, sizeof(buf));
    c = aroma_ui_card(parent, 10, y, card_w, 55, CARD_TYPE_GLASS);
    aroma_ui_label(c, "CPU", 16, 8, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    aroma_ui_label(c, buf, 16, 28, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    y += 65;

    read_meminfo("MemTotal", buf, sizeof(buf));
    c = aroma_ui_card(parent, 10, y, card_w, 55, CARD_TYPE_GLASS);
    aroma_ui_label(c, "Total Memory", 16, 8, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    aroma_ui_label(c, buf, 0, 28, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    y += 65;

    read_meminfo("MemAvailable", buf, sizeof(buf));
    c = aroma_ui_card(parent, 10, y, card_w, 55, CARD_TYPE_GLASS);
    aroma_ui_label(c, "Available Memory", 16, 8, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    aroma_ui_label(c, buf, 16, 28, LABEL_STYLE_LABEL_SMALL, state.settings_font);
    y += 65;
...
static AromaNode *build_about_page(AromaNode *parent, int panel_w, int area_h)
{
    AromaNode *sc = aroma_container_create(parent, 230, 0, panel_w, area_h);
    aroma_container_set_scrollable(sc, true);
    build_about_page_content(sc, panel_w);
    return sc;
}
void build_settings_ui(AromaNode *window)
{
    ...
    state.listview_containers[1] = build_about_page(state.settings_root, panel_w, area_h + 20);
