...
bool init_fonts(void)
{
    ...
    state.clock_font = create_font_safe(aroma_ubuntu_ttf, aroma_ubuntu_ttf_len, 72, "clock_font");
    ...
    state.drawer_icon_font = create_font_safe(icon_ttf, icon_ttf_len, 56, "drawer_icon_font");
    if (!state.drawer_icon_font) success = false;

    state.drawer_chrome_icon_font = create_font_safe(chrome_icon_ttf, chrome_icon_ttf_len, 28, "drawer_chrome_icon_font");
    if (!state.drawer_chrome_icon_font) success = false;
    return success;
}
...
void cleanup_fonts(void)
{
    ...
    if (state.drawer_icon_font) {
        aroma_ui_unload_font(state.drawer_icon_font);
        state.drawer_icon_font = NULL;
    }

    if (state.drawer_chrome_icon_font) {
        aroma_ui_unload_font(state.drawer_chrome_icon_font);
        state.drawer_chrome_icon_font = NULL;
    }
}
