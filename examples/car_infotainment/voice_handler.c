...
{
    pthread_mutex_lock(&state.voice_mutex);
    strncpy(state.voice_nav_dest, dest, sizeof(state.voice_nav_dest) - 1);
    state.voice_nav_dest[sizeof(state.voice_nav_dest) - 1] = '\0';
...
void queue_voice_partial(const char *partial_text)
{
    ...
    if (partial_text && strlen(partial_text) > 0) {
        strncpy(state.voice_partial_text, partial_text, sizeof(state.voice_partial_text) - 1);
        state.voice_partial_text[sizeof(state.voice_partial_text) - 1] = '\0';
...
{
    pthread_mutex_lock(&state.voice_mutex);
    state.voice_theme_change = dark_mode;
    pthread_mutex_unlock(&state.voice_mutex);
}
...
void queue_voice_action(int tab_index, bool call, bool end_call, const char *status)
{
    ...
    if (status) {
        strncpy(state.voice_status_text, status, sizeof(state.voice_status_text) - 1);
        state.voice_status_text[sizeof(state.voice_status_text) - 1] = '\0';
    }
    pthread_mutex_unlock(&state.voice_mutex);
}
...
void process_voice_commands(void)
{
    ...
    if (state.voice_nav_trigger) {
        ...
        for (int i = 0; i < 5; i++) {
            if (strstr(state.voice_nav_dest, cities[i].key)) {
                lat = cities[i].lat;
                lon = cities[i].lon;
                break;
            }
