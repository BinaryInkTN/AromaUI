...
{
#ifdef AROMA_USE_VOICE_CONTROL
    extern void start_voice_control_thread(void);
...
void set_voice_status(const char *status)
{
    ...
    if (state.voice_status_label && safe_node_check((const AromaNode *)state.voice_status_label, "voice_status_label")) {
        aroma_label_set_text(state.voice_status_label, status ? status : "");
    }

    if (!state.voice_status_card || !safe_node_check((const AromaNode *)state.voice_status_card, "voice_status_card")) {
        return;
    }
...
void process_voice_commands(void)
{
    ...
    if (state.voice_ac_change != 0) {
        ...
        if (state.ac_temp_label && safe_node_check((const AromaNode *)state.ac_temp_label, "ac_temp_label")) {
            char buf[16];
